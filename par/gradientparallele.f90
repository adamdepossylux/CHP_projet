		module gradientconjugue
			...
			contains
				...
				subroutine produitmat(nnz,colonnes,ic,xx,Nx,Ny,produitmatriciel,i1,iN,statinfo)
					...
					if (Np>1) then
					if (me==Np-1) then
						call MPI_ISEND(xx(i1:Nx),Nx,MPI_DOUBLE_PRECISION,me-1,tag4,MPI_COMM_WORLD,request,statinfo )
						call MPI_IRECV(pp(i1-Nx:i1-1),Nx,MPI_DOUBLE_PRECISION,me-1,tag6,MPI_COMM_WORLD,request,statinfo )
						call MPI_WAIT(request,status,statinfo)


					else if(me==0) then
						call MPI_ISEND(xx(iN-Nx+1:iN),Nx,MPI_DOUBLE_PRECISION,me+1,tag6,MPI_COMM_WORLD,request,statinfo )
						call MPI_IRECV(pp(iN+1:iN+Nx),Nx,MPI_DOUBLE_PRECISION,me+1,tag4,MPI_COMM_WORLD,request,statinfo)
						call MPI_WAIT(request,status,statinfo)
						...
						call MPI_IRECV(pp(i1-Nx:i1-1),Nx,MPI_DOUBLE_PRECISION,me-1,tag6,MPI_COMM_WORLD,request,statinfo )
						call MPI_WAIT(request,status,statinfo)
						call MPI_ISEND(xx(i1:Nx+2),Nx,MPI_DOUBLE_PRECISION,me-1,tag4,MPI_COMM_WORLD,request,statinfo )
						call MPI_IRECV(pp(iN+1:iN+Nx),Nx,MPI_DOUBLE_PRECISION,me+1,tag4,MPI_COMM_WORLD,request,statinfo )
						call MPI_WAIT(request,status,statinfo)
					end if
