...
! construction de la matrice csr
			...
			subroutine sme(x,y,Nx,Ny,i1,iN,Uinitial,t,n,nt)
				use mpi
				implicit none
				...
				double precision, dimension(:),allocatable::Uinitial,u
				...
				do k=i1,iN
					...
					if ((j==1) .OR. (j==Ny+2))  then
						...
						Uinitial(k)=ff(x(i),y(j),t(n),Ly,Lx)*dt+u(k)
					end if
				end do
			end subroutine sme


			subroutine sm1(x,y,Nx,Ny,i1,iN,ssm,u)
				use mpi
				implicit none
			...
			subroutine sm2(x,y,Nx,Ny,i1,iN,ssm,u)
				use mpi
				implicit none
				integer::Nx,Ny,i,j,k,i1,iN,d
				double precision :: dt
