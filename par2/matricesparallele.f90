...
! construction de la matrice csr
			...
			subroutine laplaciencsr(hx,hy,dt,D,ic1,colonnes,nnz,Nx,Ny,i1,iN)
				  implicit none
				  integer :: i,n,Nx,Ny,j,Njic,Nj,Njc,i1,iN
				  double precision :: dx,dy,dt,hx,hy,D
				  ...
				  double precision, dimension(:), allocatable :: colonnes,nnz,ic,ic1
					...
					ic(1)=1
					nnz(1:3)=[1+2*D*dt*(1/(hx**2))+dt*2*D*(1/(hy**2)),-dt*(D/(hx**2)),-1*dt*(D/(hy**2))]
					...
					Njic=2
					!construction de A_1
					!disparitions de g0,1 g0,2 .... g0,n-1
					do i=1,nx-2
						!on se balade de l'indice 2 à l'indice nx-1, donc on est en i+1 et non en i
						nnz((i-1)*4+Nj:i*4+nj)=[-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-dt*(D/(hx**2)),-1*dt*(D/(hy**2))]
						ic(1+Njic)=ic(Njic)+4
						colonnes((i-1)*4+Nj:i*4+nj)=[i+1-1,i+1,i+1+1,i+1+nx]
						Njic=Njic+1
					end do
					!disparition de g0,n et g0,n-1
					Nj=Nj+4*(nx-2)
					nnz(Nj:Nj+2)=[-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-1*dt*(D/(hy**2))]
					colonnes(Nj:Nj+2)=[nx-1,nx,nx+nx]
					ic(Njic+1)=ic(Njic)+3 !i=i+1, à la fin i=nx-1
					Nj=Nj+3
					Njic=Njic+1
					fg=[-1*dt*(D/(hy**2)),-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-dt*(D/(hx**2)),-1*dt*(D/(hy**2))]
					!construction de l'intérieur de la matrice B A  B
					do j=1,ny-2
							!disparition de gi,0 avec 1<i<n
							nnz(Nj:Nj+3)=[-dt*(D/(hy**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-dt*(D/(hx**2)),-1*dt*(D/(hy**2))]
							colonnes(Nj:Nj+3)=[j*nx+1-nx,j*nx+1,j*nx+1+1,j*nx+1+nx]
							ic(Njic+1)=ic(Njic)+4 !k=i*nx+j
							Nj=Nj+4
							Njic=Njic+1
							!intérieur de la matrice avec les cinq diagonales
							do i=1,nx-2
								!on se balade de l'indice 2 à l'indice nx-1, donc on est en i+1 et non en i
							...
							Nj=Nj+5*(nx-2)
							!disparition de gi,Nx+2 avec 1<i<Nx+1
							nnz(Nj:Nj+3)=[-dt*(D/(hy**2)),-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-1*dt*(D/(hy**2))]
					...
					!On répète le même principe que pour le début pour la fin de la matrice
					nnz(Nj:Nj+2)=[-1*dt*(D/(hy**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-dt*(D/(hx**2))]
					colonnes(Nj:Nj+2)=[(ny-1)*nx+1-nx,(ny-1)*nx+1,(ny-1)*nx+1+1]
					ic(Njic+1)=ic(Njic)+3 !k=i*nx+j
					Nj=Nj+3
					Njic=Njic+1

					do i=1,nx-2
						!on se balade de l'indice 2 à l'indice nx-1, donc on est en i+1 et non en i
						nnz((i-1)*4+Nj:i*4+nj)=[-1*dt*(D/(hy**2)),-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2)),-dt*(D/(hx**2))]
						ic(Njic+1)=ic(njic)+4
						colonnes((i-1)*4+Nj:i*4+nj)=[(ny-1)*nx+1+i-nx,(ny-1)*nx+1+i-1,(ny-1)*nx+1+i,(ny-1)*nx+1+i+1]
						Njic=Njic+1
					end do
					Nj=Nj+4*(nx-2)
					nnz(Nj:Nj+2)=[-dt*(D/(hy**2)),-dt*(D/(hx**2)),1+2*dt*(D/(hx**2))+dt*2*(D/(hy**2))]
